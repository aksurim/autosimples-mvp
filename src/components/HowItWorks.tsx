...
export function HowItWorks() {
  const steps = [
    ...
    {
      icon: <CalendarCheck className="w-8 h-8 text-brand-teal" />,
        ...
        <div className="grid md:grid-cols-3 gap-12">
          {steps.map((step, index) => (
            <div key={index} className="relative flex flex-col items-center text-center group">
              <div className="w-20 h-20 bg-brand-light rounded-full flex items-center justify-center mb-6 group-hover:bg-brand-teal/10 transition-colors">
                {step.icon}
              </div>
